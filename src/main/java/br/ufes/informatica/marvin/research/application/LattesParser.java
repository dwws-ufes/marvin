...
class LattesParser implements PublicationInfo {
  ...
  public LattesParser(InputStream inputStream) throws LattesParseException {
    try {
      ...
      if (CONFIG == null) {
        logger.log(Level.INFO, "Loading Lattes parser configuration from file: {0}",
            CONFIG_FILE_NAME);
        CONFIG = new Properties();
        String parserConfigFileName =
            getClass().getPackage().getName().replaceAll("\\.", "/") + "/" + CONFIG_FILE_NAME;
        File parserConfigFile = ResourceUtil.getResourceAsFile(parserConfigFileName);
        logger.log(Level.FINE, "Lattes parser configuration file located at: {0}",
            parserConfigFile.getAbsolutePath());
        CONFIG.load(new FileInputStream(parserConfigFile));
      }

      // Reads the contents of the input stream (i.e., the XML file) into a string buffer ready for
      // Jsoup.
      try (BufferedReader reader =
          new BufferedReader(new InputStreamReader(inputStream, CONFIG.getProperty("encoding")))) {
      ...
      logger.log(Level.INFO, "Read {0} characters from the given CV's input stream.",
          xmlContents.length());
    } catch (Exception e) {
      // In case there's any exception that prevents us from reading the CV, wraps it in an
      // exception the controller can
      // handle.
      logger.log(Level.SEVERE,
          "Exception while trying to read the contents of the XML file that holds the Lattes CV.",
          e);
  ...
  public void parse() throws LattesParseException {
    ...
    logger.log(Level.FINE, "Jsoup successfully parsed the CV # {0}, of {1}",
        new Object[] {lattesId, researcherName});

    // Navigates to the nodes that contains the production (supports multiple, although we expect a
    // single one).
  ...
  private SortedSet<LattesEntry> extractEntries(Element element, String name) {
    ...
    for (Element elem : elems) {
      ...
      LattesEntry entry =
          new LattesEntry(type, year, title, doi, venues.toString(), authors.toString());

      // Check if any of the optional attributes are present.
      if (attrPageStart != null && detail.hasAttr(attrPageStart))
        entry.setPageStart(detail.attr(attrPageStart));
      if (attrPageEnd != null && detail.hasAttr(attrPageEnd))
        entry.setPageEnd(detail.attr(attrPageEnd));
      if (attrPublisher != null && detail.hasAttr(attrPublisher))
        entry.setPublisher(detail.attr(attrPublisher));
      if (attrExtra01 != null && detail.hasAttr(attrExtra01))
        entry.setExtra01(detail.attr(attrExtra01));
      if (attrExtra02 != null && detail.hasAttr(attrExtra02))
        entry.setExtra02(detail.attr(attrExtra02));
      if (attrExtra03 != null && detail.hasAttr(attrExtra03))
        entry.setExtra03(detail.attr(attrExtra03));
  ...
  private int parseYear(String yearData) {
    if (yearData.matches("\\d{4}"))
      return Integer.parseInt(yearData);
    String[] data = yearData.split(" ");
    for (int i = 0; i < data.length; i++)
      if (data[i].matches("\\d{4}"))
  ...
  private void extractArticles(SortedSet<LattesEntry> entries) {
    ...
    for (LattesEntry entry : entries) {
      JournalPaper article = new JournalPaper(entry.getTitle(), entry.getYear(), entry.getPages(),
          entry.getDoi(), entry.getPublisher(), entry.getVenue(), entry.getExtra02(),
          entry.getExtra03(), entry.getExtra01());
  ...
  private void extractBooks(SortedSet<LattesEntry> entries) {
    books = new TreeSet<>();
    for (LattesEntry entry : entries) {
      Book book = new Book(entry.getTitle(), entry.getYear(), entry.getPages(), entry.getDoi(),
          entry.getPublisher(), entry.getExtra01());
  ...
  private void extractInCollections(SortedSet<LattesEntry> entries) {
    ...
    for (LattesEntry entry : entries) {
      BookChapter chapter = new BookChapter(entry.getTitle(), entry.getYear(), entry.getPages(),
          entry.getDoi(), entry.getPublisher(), entry.getVenue(), entry.getExtra01());
  ...
  private void extractInProceedings(SortedSet<LattesEntry> entries) {
    ...
    for (LattesEntry entry : entries) {
      ...
      ConferencePaper paper =
          new ConferencePaper(entry.getTitle(), entry.getYear(), entry.getPages(), entry.getDoi(),
              entry.getPublisher(), entry.getExtra01(), entry.getVenue(), fullPaper);
