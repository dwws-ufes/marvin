...
public class GenerateBibliographyController extends JSFController {
	...
	public StreamedContent getBibliographyFile() {
		try (InputStream in = new ByteArrayInputStream(bibliography.toString().getBytes("UTF-8"))) {
			...
			// Deprecated, replaced with the next line: return new DefaultStreamedContent(in, "text/plain", "bibliography.bib");
			return DefaultStreamedContent.builder().stream(() -> in).contentType("text/plain").name("bibliography.bib").build();
		}
