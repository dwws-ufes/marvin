...
import br.ufes.informatica.marvin.core.application.InstallSystemService;
...
public class InstallSystemController extends JSFController {
  ...
  private static final Logger logger =
      Logger.getLogger(InstallSystemController.class.getCanonicalName());
  ...
  public void suggestShortName() {
    ...
    if ((name != null) && ((shortName == null) || (shortName.length() == 0))) {
      ...
      logger.log(Level.FINE, "Suggested \"{0}\" as short name for \"{1}\"",
          new Object[] {admin.getShortName(), name});
    } else
  ...
  private boolean checkPasswords() {
    if (((repeatPassword != null) && (!repeatPassword.equals(admin.getPassword())))
        || ((repeatPassword == null) && (admin.getPassword() != null))) {
      ...
      addGlobalI18nMessage("msgsCore", FacesMessage.SEVERITY_WARN,
          "installSystem.error.passwordsDontMatch.summary",
          "installSystem.error.passwordsDontMatch.detail");
      return false;
    }
    return true;
  }
   ...
   */
  public void begin() {
    logger.log(Level.FINEST, "Beginning conversation. Current conversation transient? -> {0}",
  ...
  /**
   * Registers the administrator as one of the steps of system installation and moves to the next
   * step.
   * 
  ...
  public String saveConfig() {
    logger.log(Level.FINEST,
        ...
        new Object[] {admin.getName(), admin.getEmail()});
    logger.log(Level.FINEST,
        "Received input data:\n\t- config.institutionAcronym = {0}\n\t- config.smtpServer:Port = {1}:{2}\n\t- config.smtpUsername = {3}",
        new Object[] {config.getInstitutionAcronym(), config.getSmtpServerAddress(),
            config.getSmtpServerPort(), config.getSmtpUsername()});

    // Lastly, sets the base URL for this installation of Marvin.
    ExternalContext ectx = FacesContext.getCurrentInstance().getExternalContext();
    String baseURL = "http://" + ectx.getRequestServerName() + ":" + ectx.getRequestServerPort()
    ...
    try {
      ...
      addGlobalI18nMessage("msgsCore", FacesMessage.SEVERITY_FATAL,
          "installSystem.error.operationFailed.summary",
          "installSystem.error.operationFailed.detail");
      return null;
    }
