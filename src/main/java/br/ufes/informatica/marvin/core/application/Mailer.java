...
public class Mailer implements Serializable {
   ...
   * @throws MailerException
   */
  public void sendEmail(String emailAddress, MailerTemplate mailerTemplate,
      Map<String, Object> dataModel) throws MailerException {
    try {
      // Loads the e-mail template file.
      JtwigTemplate template = JtwigTemplate
          .classpathTemplate(TEMPLATE_FOLDER_PATH + mailerTemplate + TEMPLATE_FILE_EXTENSION);
      ...
      // Sends the message.
      logger.log(Level.INFO, "Sending message using template {0} to e-mail {1}. Subject: {2}",
          new Object[] {mailerTemplate + TEMPLATE_FILE_EXTENSION, emailAddress, subject});
      ...
      email.setAuthenticator(
          new DefaultAuthenticator(config.getSmtpUsername(), config.getSmtpPassword()));
      ...
      logger.log(Level.INFO,
          "Successfuly sent message using template {0} to e-mail {1}. Subject: {2}",
          new Object[] {mailerTemplate + TEMPLATE_FILE_EXTENSION, emailAddress, subject});
    } catch (Exception e) {
      throw new MailerException(e);
    }
