 ...
 * is logged in and the current date/time.
 * 
...
public class SessionController extends AdminSession implements Serializable {
  ...
  /**
   * Provides the expiration date/time for the user session. This makes it possible to warn the user
   * when his session will expire.
   * 
    ...
    try {
      ...
      switch (e.getReason()) {
        ...
        default:
          ...
          // administrators.
          logger.log(Level.INFO, "System failure during login. Email: \"" + email + "\"; reason: \""
              + e.getReason() + "\"", e);
