 ...
 * 
 * This use case allows any user to supply an e-mail address and ask Marvin to reset her password.
 * Marvin then sends an e-mail with a code embeded in a URL. When the user opens that URL, she comes
 * back to Marvin to register her new password.
 * 
 * This controller is conversation scoped, beginning the conversation in method checkCode() and
 * ending in method end(), which should be called explicitly by the web pages.
 *
...
public class ChangePasswordController extends JSFController {
  ...
  private static final Logger logger =
      Logger.getLogger(ChangePasswordController.class.getCanonicalName());
   ...
   */
  public void end() {
    logger.log(Level.FINEST, "Ending conversation. Current conversation transient? -> {0}",
        new Object[] {conversation.isTransient()});
    ...
    if (!conversation.isTransient())
      ...
      try {
        ...
        logger.log(Level.WARNING,
            "Trying to end transient conversation usually means the user is reloading the page after the conversation has already ended. Tried to redirect back home, but failed.",
            e);
      }
  }
  ...
  private boolean checkPasswords() {
    if (repeatPassword == null || password == null || (!repeatPassword.equals(password))) {
      ...
      addGlobalI18nMessage("msgsCore", FacesMessage.SEVERITY_WARN,
          "changePassword.error.passwordsDontMatch.summary",
          "changePassword.error.passwordsDontMatch.detail");
      return false;
    }
    return true;
  }
  ...
  /**
   * Controller method that starts the password reset process by asking the service class to send
   * the e-mail with the code to the given address.
   */
  public void resetPassword() {
    ...
    addGlobalI18nMessage("msgsCore", FacesMessage.SEVERITY_INFO,
        "changePassword.message.resetRequested.summary", new Object[] {},
        "changePassword.message.resetRequested.detail", new Object[] {email});
  }

  /**
   * Controller method that asks the service class to actually change the password after the code
   * and the new password have been checked.
   * 
   * @return The path to the web page that shows that the operation completed successfully.
   */
  public String setNewPassword() {
    logger.log(Level.INFO, "Setting new password for academic {0} (password code {1})",
        new Object[] {academic, validatedPasswordCode});
    ...
    try {
      ...
      addGlobalI18nMessage("msgsCore", FacesMessage.SEVERITY_FATAL,
          "changePassword.error.operationFailed.summary",
          "changePassword.error.operationFailed.detail");
      return null;
    }
